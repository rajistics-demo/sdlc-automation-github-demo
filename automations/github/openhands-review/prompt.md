...
If started manually without a pull request event, check only the repository, instructions, and GitHub access. Do not change a pull request or its labels. Mark the readiness check successful when those checks pass, and say clearly that the event workflow was not exercised.
