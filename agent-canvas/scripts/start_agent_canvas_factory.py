#!/usr/bin/env python3
# Launches the parent Agent Canvas supervisor conversation for a story request.
...
def protected_macos_path(repo: Path) -> bool:
    ...
    try:
        ...
        return False
    parts = rel.parts
    return bool(parts and parts[0] in MACOS_PROTECTED_DIRS)


def assert_canvas_readable_repo(repo: Path, *, allow_protected_repo: bool) -> None:
    if allow_protected_repo or not protected_macos_path(repo):
        return
    raise SystemExit(
        "Refusing to start Agent Canvas from a macOS protected folder. "
        "Use a checkout under /private/tmp or a non-protected developer folder, "
        "or pass --allow-protected-repo if local Full Disk Access is already configured."
    )
...
def start_supervisor(args: argparse.Namespace) -> dict[str, Any]:
    ...
    assert_canvas_readable_repo(repo, allow_protected_repo=args.allow_protected_repo)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--allow-protected-repo", action="store_true", help="allow starting from macOS protected folders when Full Disk Access is configured")
